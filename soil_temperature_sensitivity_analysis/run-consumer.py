                ...
                for data in msg.get("data", []):
                    ...
                    if models["DEC"]:
                        with open(f"{path_to_out}/SoilTemperature_MO_DEC_{loc}_{soil}_{lai}_{aw}.txt", "w") as _:
                            ...
                            for vals in results:
                                _.write(
                                    f"{vals['Date']}, 0, 0, {vals['AMEI_DSSAT_EPICST_standalone_SurfTemp']}, na, na\n"
                                )
                                upper_cm = 0
                    ...
                    if models["SAC"]:
                        with open(f"{path_to_out}/SoilTemperature_MO_SAC_{loc}_{soil}_{lai}_{aw}.txt", "w") as _:
                            ...
                            for vals in results:
                                _.write(
                                    f"{vals['Date']}, 0, 0, {vals['AMEI_Simplace_Soil_Temperature_SurfTemp']}, na, na\n"
                                )
                                upper_cm = 0
                    ...
                    if models["SQC"]:
                        with open(f"{path_to_out}/SoilTemperature_MO_SQC_{loc}_{soil}_{lai}_{aw}.txt", "w") as _:
                            ...
                            for vals in results:
                                ...
                                _.write(
                                    f"{vals['Date']}, 0, 5, {round((st_min + st_max) / 2.0, 6)}, {st_max}, {st_min}\n"
                                )
                                layer_depths = [
                                    (5, 15),
                                    (15, 30),
                                ...
                                for upper_cm, lower_cm in layer_depths:
                                    _.write(
                                        f"{vals['Date']}, {upper_cm}, {lower_cm}, {vals['AMEI_SQ_Soil_Temperature_SoilTemp_deep']}, na, na\n"
                                    )
                        ...
                        with open(f"{path_to_out}/SoilTemperature_MO_APC_{loc}_{soil}_{lai}_{aw}.txt", "w") as _:
                            ...
                            for vals in results:
                                ...
                                for i, s_temp in enumerate(vals["AMEI_ApsimCampbell_SoilTemp"]):
                                    ...
                                    lower_cm = upper_cm + lt_cm
                                    _.write(
                                        f"{vals['Date']}, {upper_cm}, {lower_cm}, {s_temp}, {vals['AMEI_ApsimCampbell_SoilTemp_max'][i]}, {vals['AMEI_ApsimCampbell_SoilTemp_min'][i]}\n"
                                    )
                                    upper_cm = lower_cm
