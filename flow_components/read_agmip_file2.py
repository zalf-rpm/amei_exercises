...
class ReadAgmipFile(process.Process):
    def __init__(self, metadata: dict[str, Any] | None, con_man: common.ConnectionManager = None):
    ...
    async def run(self):
        ...
        enabled_sheets.update(map(lambda k: (k, True), self.config.get("enabled_sheets", [])))
        ...
        agmip_elem_to_schema_elem.update(self.config.get("agmip_elem_to_schema_elem", {}))
        weather_elements = set(agmip_elem_to_schema_elem.keys())
        conf_weather_elements = self.config.get("weather_elements", [])
        if len(conf_weather_elements) > 0:
            weather_elements = weather_elements.intersection(self.config.get("weather_elements", []))
        ...
        if self.out_ports["out"]:
            try:
                ...
                logger.error(f"{os.path.basename(__file__)}: {self.config['name']} RPC Exception: {e.description}")

        logger.info(f"{self.name} process finished")


def main():
    process.run_process_from_metadata_and_cmd_args(ReadAgmipFile(meta), meta)
